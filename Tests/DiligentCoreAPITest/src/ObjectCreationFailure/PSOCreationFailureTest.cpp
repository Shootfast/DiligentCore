...
/*
TEST_F(PSOCreationFailureTest, VSinMeshPipeline)
{
    if (!HasMeshShader())
        GTEST_SKIP();

    auto PsoCI{GetMeshPSOCreateInfo("PSO Create Failure - VS in Mesh pipeline")};
    PsoCI.pVS = GetVS();
    TestCreatePSOFailure(PsoCI, "Vertex, geometry and tessellation shaders are not supported in a mesh pipeline");
}
*/
